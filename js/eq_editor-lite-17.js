...
function setadvert() {
    ...
    var x = e.offsetWidth;
    if (CCgetId('wrap').style.marginLeft == 'auto')
        x -= 170;
...
function EqTextArea(preview, input, comment, download, intro) {
    ...
    this.updateExportArea = function () {
        for (i = 0; i < this.bsize; i++) {
            ...
            if (this.bArray_area[i].src !== undefined)
                this.bArray_area[i].src = v;
    ...
    this.addEvent = function (action, fn) {
        if (this.equation_input.addEventListener)
            ...
            this.equation_input.attachEvent('on' + action, fn);
    };
    this.set = function (preview, input, comment, download, intro) {
        if (preview == undefined || preview == '')
            preview = 'equationview';
        if (input == undefined || input == '')
            input = 'latex_formula';
        if (comment == undefined || comment == '')
            comment = 'equationcomment';
    ...
    this.setText = function (val) {
        ...
        do {
            ...
            if (i != -1) {
                ...
                if (EqEditor.setSelIndx('fontsize', subtex))
                    go = 1;
                if (subtex == '\\inline') {
                ...
                if (subtex.substr(0, 5) == '\\dpi{' && EqEditor.setSelIndx('dpi', subtex.substr(5)))
                    go = 1;
                if (go)
                    latex = latex.substr(i + 1);
            }
        }
        while (go)
        if (latex.length > 0) {
    ...
    this.getLaTeX = function () {
        ...
        if (a.length > 0)
            return EqEditor.getSize() + a;
        return '';
    };
    this.getEquationStr = function () {
        var a = this.getLaTeX();
        if (a.length > 0)
            return EqEditor.getCompressed() + EqEditor.getDPI() + EqEditor.getBG() + EqEditor.getFont() + a;
        return '';
    };
        ...
        case 'html':
            {
                ...
                var a = this.exportEquation('safe');
                if (format == 'swf')
                    return AC_FL_RunContent('codebase', EDITOR_SW_FLASH, 'width', '600', 'height', '100', 'src', (EQUATION_ENGINE + '/swf.latex?' + a), 'quality', 'high', 'pluginspage', EDITOR_SW_PLAYER, 'align', 'top', 'scale', 'showall', 'wmode', 'window', 'devicefont', 'false', 'bgcolor', '#ffffff', 'menu', 'true', 'allowFullScreen', 'true', 'movie', (EQUATION_ENGINE + '/swf.latex?' + a));
                else
                    return ('<img src="' + EQUATION_ENGINE + '/' + format + '.latex?' + a + '" title="' + this.getLaTeX() + '" />');
            }
            break;
        default:
            {
    ...
    this.setdownload = function (text) {
        if (this.equation_download)
            this.equation_download.innerHTML = text;
    };
    this.setcomment = function (text) {
        if (this.equation_comment)
            this.equation_comment.innerHTML = text;
    };
    this.renderEqn = function (callback) {
        ...
        if (bracket == 0) {
            if (CCgetId('renderbutton'))
                CCgetId('renderbutton').className = 'greybutton';
            ...
            if (bracket < 0)
                this.setcomment("<br/><span class=\"orange\">You have more <strong>closed '}' brackets</strong> than open '{' brackets</span>");
            else
                this.setcomment("<br/><span class=\"orange\">You have more <strong>open '{' brackets</strong> than closed '}' brackets</span>");
        }
        this.changed = false;
    };
    this.clickval = 0;
    ...
    this.undo = function (box) {
        ...
        if (this.myRedo < this.myUndo) {
            this.myRedo++;
            if (this.myRedo == this.myUndo && CCgetId('undobutton'))
                CCgetId('undobutton').src = EDITOR_SRC + "/plugins/demo/img/undo-x.gif";
            var a = CCgetId('redobutton');
            if (a) a.src = EDITOR_SRC + "/plugins/demo/img/redo.gif";
        } else return;
    ...
    this.redo = function (box) {
        if (this.myRedo > 0) {
            this.myRedo--;
            if (this.myRedo == 0 && CCgetId('redobutton'))
                CCgetId('redobutton').src = EDITOR_SRC + "/plugins/demo/img/redo-x.gif";
            var a = CCgetId('undobutton');
            if (a) a.src = EDITOR_SRC + "/plugins/demo/img/undo.gif";
        } else
            return;
...
var Example = {
    ...
    show: function (button, group) {
        ...
        if (group == 'fav' || group == 'history') {
            ...
            gallery.init('photos', 'leftarrow', 'rightarrow', 'overview', FAVORITE_ENGINE + '/example_json.php?fn=gallery');
    ...
    state: function (state) {
        for (i = 0; i < this.bsize; i++) {
            if (state) this.bArray_id[i].className = 'lightbluebutton';
                ...
                this.bArray_id[i].className = 'greybutton';
        }
    },
...
var EqEditor = {
    ...
    add: function (obj, resize) {
        ...
        if (resize) {
            if (window.addEventListener)
                window.addEventListener('resize', new Function('EqEditor.resize(' + this.targetSize + ');'), false);
            else
                window.attachEvent('onresize', new Function('EqEditor.resize(' + this.targetSize + ');'));
            EqEditor.resize(this.targetSize);
        }
        if (this.targetSize == 0)
            obj.equation_input.focus();
        this.targetSize++;
    },
    editor_id: null,
    ...
    textchanged: function () {
        if (this.targetArea.textchanged())
    ...
    countclick: function () {
        ...
        var a = CCgetId('redobutton');
        if (a) a.src = EDITOR_SRC + "/plugins/demo/img/redo-x.gif";
        a = CCgetId('undobutton');
        if (a)
            a.src = EDITOR_SRC + "/plugins/demo/img/undo.gif";
    },
    ...
    keyHandler: function (e) {
        ...
        if (keystr == this.extendchar)
            this.backCursor(this.equation_input);
...
Scroll.prototype = {
    ...
    add_panel: function () {
        if (this.ajax_php && this.ajax_response_fn) {
            if (this.ajax_php.indexOf("?") == -1)
                loadXMLDoc(this.ajax_php + '?panel=' + this.panels + this.subtext, this.ajax_response_fn);
            else
                loadXMLDoc(this.ajax_php + '&panel=' + this.panels + this.subtext, this.ajax_response_fn);
    ...
    redraw: function () {
        ...
        if (this.ajax_php && this.ajax_response_fn) {
            var obj = this;
            if (this.ajax_php.indexOf("?") == -1)
                loadXMLDoc(obj.ajax_php + '?panel=' + this.panels + this.subtext, obj.ajax_response_fn);
            else
                loadXMLDoc(obj.ajax_php + '&panel=' + this.panels + this.subtext, obj.ajax_response_fn);
        } else if (this.json_php) {
            var a = this.panels;
